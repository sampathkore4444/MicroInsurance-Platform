...
# ---- CREATE POLICY ----
    ...
    if customers:
        ...
        cumulative_goal = st.number_input(
            "Cumulative Savings Goal (KES)",
            min_value=1000.0,
            value=45000.0,
            step=1000.0,
        )
        duration_years = st.number_input(
            "Duration (Years)", min_value=1, value=3, step=1
        )
        payment_freq = st.selectbox("Payment Frequency", ["Daily", "Weekly", "Monthly"])
        coverage = st.selectbox("Coverage Type", ["Life & Critical Illness"])
        currency = st.selectbox("Payment Currency", ["KES", "USD"])
        ...
        payments_per_year = {"Daily": 365, "Weekly": 52, "Monthly": 12}[payment_freq]
        total_payments = payments_per_year * duration_years
        per_payment = cumulative_goal / total_payments

        st.info(
            f"Per-payment contribution: {per_payment:.2f} {currency} ({total_payments} payments over {duration_years} years)"
        )

        if st.button("Create Policy"):
            c.execute(
                "INSERT INTO policies (customer_id,savings_amount,cumulative_goal,coverage_type,payment_frequency,payment_currency,created_at) VALUES (?,?,?,?,?,?,?)",
                (
                    customer_dict[selected_customer],
                    per_payment,
                    cumulative_goal,
